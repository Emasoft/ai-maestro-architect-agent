...
Before planning, read the board and check whether a card already exists:
`ama-kanban-render` (the board) and `ama-trdd-find` (search). A plan that
duplicates an existing TRDD is worse than no plan — it splits the work in two
places and neither is trustworthy afterwards.
