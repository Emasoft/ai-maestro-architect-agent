...
find_publish_ancestor() {
    ...
    while [ -n "${pid}" ] && [ "${pid}" != "0" ] && [ "${pid}" != "1" ] && [ "${depth}" -lt 15 ]; do
        ...
        # ONE alternative, deliberately. A second `*python*/scripts/publish.py*`
        # was here and was DEAD: the leading `*` already spans the slash, so
        # `*scripts/publish.py*` matches every command the slashed form would,
        # and shellcheck flagged it (SC2221/SC2222 — "always overrides a later
        # one" / "never matches"). Removing it accepts and rejects exactly what
        # it did before — the surviving pattern is the strict superset — so this
        # does not widen or narrow the gate. Do not "restore" the slashed form:
        # it can only ever be unreachable.
        case "${cmd}" in
...
Examples:
  ...
  %(prog)s --patch --dry-run    # run every validation step fully, then
    ...
    if info.has_kind(ProjectKind.CLAUDE_PLUGIN):
        ...
        # CPV is PINNED (@v5.4.0) — see the BUMP PROTOCOL note on Step 5. An
        ...
        # did not — fixed upstream in CPV v3.19.0, which is why the pin could
        # move). Pinning restores a deterministic gate; it does NOT weaken it.
        run([
    ...
    if info.has_kind(ProjectKind.CLAUDE_PLUGIN):
        ...
        # CPV is PINNED (@v5.4.0). BUMP PROTOCOL: this pin lives at FOUR sites —
        ...
        # EVERY distinct invocation is verified clean at the new tag — there are
        # THREE, not one: `lint <root>`, `plugin . --strict`, and validate.yml's
        # `plugin . --strict --verbose --report <file>`. Verifying only the
        # `--strict` form leaves two of the four sites unevidenced (2026-08-14:
        # caught exactly that on the 2.153.1 → 5.4.0 bump). A deliberate,
        # greppable bump beats silent drift onto a moving branch. Same
        # supply-chain fix the janitor applied to its own CPV pin.
        run([
            "uvx", "--from", "git+https://github.com/Emasoft/claude-plugins-validation@v5.4.0",
    ...
    staged: list[str] = []
    # uv.lock is regenerated by _sync_uv_lock() during the version bump so it
    # tracks the new version. Without staging it here, the release commit ships
    # a stale lockfile and the next run hits the Step 1 dirty-tree branch and
    # cuts an extra `chore: update uv.lock` housekeeping commit (issue #10).
    for name in (
