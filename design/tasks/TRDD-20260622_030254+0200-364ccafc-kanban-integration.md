...
updated: 2026-06-22T10:57:40+0200
current-owner: amaa
assignee: amaa
priority: 3
severity: MEDIUM
...
**NEXT ACTION (remaining, doable HERE):** (a) wire both ops into `amaa-design-lifecycle` PROCEDURE 5 (`references/procedures.md` — create the epic on COMPLETED, feed its id to the handoff `aimaestro_task_id`, optional progress-query during tracking); (b) add contract tests asserting the BUILT command strings (no server); (c) file the 2 ai-maestro CLI Method-1 follow-ups (`--attachments` on create, `--parent` on list). DEPLOYMENT-time / post-ship: LIVE round-trip (Phase 0), orchestrator read-side issue, optional secondary-copy consolidation.
