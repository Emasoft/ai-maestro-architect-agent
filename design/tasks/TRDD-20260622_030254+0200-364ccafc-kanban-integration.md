...
updated: 2026-06-22T10:55:37+0200
current-owner: amaa
assignee: amaa
priority: 3
severity: MEDIUM
...
**PHASE 2 PROGRESS (2026-06-22):** `op-create-kanban-epic.md` AUTHORED in BOTH locations (`amaa-design-communication-patterns-ops/references/` + the non-ops twin), matching the op-* format, with the verified `amp-kanban-create-task --task-type epic … --parent` command construction + error-handling table.

**NEXT ACTION (remaining Phase 2/3/4, all doable HERE):** (1) author `op-query-kanban-progress.md` (+ twin) — `amp-kanban-list` → client-side `parentTask==<epicId>` filter; (2) wire both ops into `amaa-design-lifecycle` PROCEDURE 5 (`references/procedures.md`: create the epic on COMPLETED → feed its id to the handoff's `aimaestro_task_id`); (3) add contract tests asserting the BUILT command strings (no live server). DEPLOYMENT-time / post-ship: the LIVE round-trip (Phase 0), the orchestrator read-side Method-1 issue, the optional secondary-copy consolidation.
