...
updated: 2026-06-22T10:49:59+0200
current-owner: amaa
assignee: amaa
priority: 3
severity: MEDIUM
...
**NEXT ACTION — Phase 2 (doable HERE, no live agent needed):** author two ops + contract-test their command construction — `op-create-kanban-epic` (`amp-kanban-create-task "Design: <proj>" --task-type epic --labels design,epic --description <doc>` → capture id → first-level children with `--parent <epicId>`) and `op-query-kanban-progress` (`amp-kanban-list` → client-side `parentTask==<epicId>` filter). Then wire both into the `amaa-design-lifecycle` flow (epic on design-complete; query on demand). Contract tests assert the BUILT command strings (no server). DEPLOYMENT-time / post-ship: the LIVE round-trip (Phase 0), the orchestrator read-side Method-1 issue, the optional secondary-copy consolidation.
