...
To report to AMCOS, send a message using the `amp-send` CLI with:
...
| Can't send message | AI Maestro messaging unavailable | Verify AI Maestro is running, then use `amp-send` |
| AMCOS not responding | AMCOS session crashed | Report to system admin (user) |
| Missing context | Task too vague | Send [QUESTION] message to AMCOS using `amp-send` |
...
| Script | Purpose |
|--------|---------|
| `scripts/publish.py` | Release pipeline — bypass-guard → lint → remote CPV `--strict` validate → tests → bump → changelog → commit → push → release; installs `.githooks/pre-push` |
| `scripts/amaa_design_lifecycle.py` | Design-document state machine (incl. the `IMPLEMENTING → REVIEW` redesign loop) |
| `scripts/amaa_design_validate.py` | Validate the architect's own design documents (frontmatter, UUID, status) |
| `scripts/amaa_requirement_analysis.py` | Requirement analysis tooling |
| `scripts/amaa_download.py` | Plugin download utility |

Plugin validation is **not vendored locally** — it runs only via the remote CPV
launcher (`uvx cpv-remote-validate plugin . --strict`).

---

## 17. Change History

The authoritative changelog is `CHANGELOG.md` (generated by `git-cliff` on each
`publish.py` release). Notable architecture shifts:

- **v2.7.0** — removed all vendored local plugin-validators; validation runs only
  via the remote CPV launcher. Pre-push gate is `publish.py`-managed
  (`.githooks/pre-push`).
- **v2.6.0** — adopted the R6 v3 communication model and the v2 `column:` task
  pipeline; the legacy 5-column kanban is **superseded** (see
  `skills/amaa-prrd-trdd-kanban/SKILL.md` and `FULL_PROJECT_WORKFLOW.md`).
...
| (AI Maestro) | AI Maestro messaging system (AMP) | AMP handles routing automatically; use the `amp-*` CLIs (`amp-send`, `amp-inbox`) for all messaging |
